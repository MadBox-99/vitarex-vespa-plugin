...
function vespa_szabadidos_register()
{
    ...
    // Nem cél a teljes védelem; a kísérletszámot korlátozza hálózatonként.
    //
    // A számláló a validáció ELŐTT növel, tehát a hibásan kitöltött űrlap is fogyaszt
    // a keretből. Ezért rövid az ablak: aki elgépeli magát, pár perc múlva újrapróbálhatja,
    // a tömeges visszaélés viszont továbbra is falba ütközik.
    ...
    if ($probak >= 5) {
        wp_send_json_error(array('message' => 'Túl sok regisztrációs kísérlet erről a hálózatról. Kérjük, próbáld újra néhány perc múlva.'));
    }
    set_transient($dobbanto_kulcs, $probak + 1, 10 * MINUTE_IN_SECONDS);
